...
def _get_board(obs) -> np.ndarray:
    """Extrae la matriz del tablero de la observación (dict o ndarray)."""
    if isinstance(obs, dict):
        board = obs.get("board", obs.get("matrix", None))
        if board is None:
            # Último recurso: tomar el primer array del dict
...
def _column_heights(board: np.ndarray) -> np.ndarray:
    """Altura de cada columna (número de celdas ocupadas desde la base)."""
    H, W = board.shape
...
def tetris_features(obs) -> np.ndarray:
    ...
    Features:
      ...
      1 – max_height        (altura máxima)
      2 – holes             (celdas vacías bajo celdas ocupadas)
      3 – bumpiness         (suma |h_i - h_{i+1}|)
      4 – std_heights       (desviación estándar de alturas)
      5 – complete_lines    (filas completamente llenas)
      6 – wells             (número de "pozos" laterales)

    Cada feature se normaliza al rango [0,1] con divisores heurísticos.
    """
