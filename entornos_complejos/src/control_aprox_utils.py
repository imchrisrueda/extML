...
def report_train_eval_summary(train_arr, eval_arr, color, label, title, final_window, decimals, smooth_w, extra_hlines=None):
    """Muestra una tabla-resumen y luego el gráfico train/eval de una configuración."""
    ...
    gap = eval_mean - train_mean
    best_ep = best_smoothed_episode(train_arr, smooth_w)
    fmt = f"{{:.{decimals}f}}"
    ...
    summary_df = pd.DataFrame(
        [
            {
                "Método": label,
                "Ventana final": final_window,
                "Train media final": fmt.format(train_mean),
                "Train std semillas": fmt.format(train_std),
                "Gap eval-train": f"{gap:+.{decimals}f}",
                "Mejor ep (MA)": best_ep,
                "Eval greedy media": fmt.format(eval_mean),
                "Eval greedy std": fmt.format(eval_std),
                "Eval n_seeds": int(eval_arr.shape[0]),
                "Eval n_episodes": int(eval_arr.shape[1]),
            }
        ]
    )
